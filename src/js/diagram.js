...
function getMaxY(data, previousData) {
    var max = 0;
...
function drawLinearGraph(selector, data, previousData, options) {
    ...
    for(var i = 0; i < getMaxY(data, previousData); i += options.dY) {
		c.fillStyle = '#808080';
		console.log(i, getYPixel(i));
        c.fillText(i, options.xPadding - 10, getYPixel(i)-20);
    }
	...
	function getYPixel(val) {
	    return graph.height() - (((graph.height() -  options.yPadding) / getMaxY(data, previousData)) * val) -  options.yPadding;
	}
}
...
$(function() {
	...
	window.drawStocksPlot = function(areaSelector) {
		...
		drawLinearGraph(areaSelector, plotData[0], plotData[1], options);
	}
});